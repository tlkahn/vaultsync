...
| 2026-08-31 | **v2.2 (PR50-r2 5476798257):** H1-V conditional etag now prefers the GET entity etag when the validate probe returns one (warm-load authority); a HEAD vs GET etag mismatch fails closed (`Err`, caller F2 policy); GET-etag-less backend falls back to HEAD (residual). D-cond updated. |
