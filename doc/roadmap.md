...
6. Integration test optional gate in CI (**implemented on branch
   `ci/issue-6`, issue 6** - `VAULTSYNC_TEST_S3_REQUIRE=1` no-silent-skip
   sentinel in the harness; `integration` PR-gate job (suite minus the
   paginate test until #17) + `integration-nightly` full-suite cron job, both
   OIDC-authenticated against a dedicated test bucket under `ci/` with a
   7-day lifecycle backstop; `timeout-minutes: 20` hang guard)
7. CI: pin the toolchain and verify MSRV (**done, issue 5** - `rust-toolchain.toml` pins 1.95.0; `rust-version = "1.95"` verified against the final dep tree in CI by the `msrv` job; fmt/clippy/test on ubuntu+macos via rust-cache). The B-L7 remainder (bucket env var in CI + no-silent-skip hardening) is item 6 / issue #6; the paginate-test slowness gating the full suite is #17
