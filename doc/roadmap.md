...
7. CI: pin the toolchain and verify MSRV (**done, issue 5** - `rust-toolchain.toml` pins 1.95.0; `rust-version = "1.95"` verified against the final dep tree in CI by the `msrv` job; fmt/clippy/test on ubuntu+macos via rust-cache). What remains, split across #6 and #17: set `VAULTSYNC_TEST_S3_BUCKET` in CI so the env-gated suite genuinely runs, and use `#[ignore]` + `--ignored` or a CI sentinel so a silent skip cannot look green (B-L7 hardening) - the integration suite currently hangs in CI and is gated out of the test jobs pending #17
